...
#################################################
def romExpand32Banks():
  """ This patches the rom before encoding, to add extra space. """
...
#################################################
def romExpandMoveMapTilesets():
  """ Handles clearing the old area, and rewriting assembly. """

  bank = mystic.romSplitter.banks[0xc]
  for i in range(0,0x4000):
    bank[i] = 0xff

  bank = mystic.romSplitter.banks[0xb]
...
def romExpandMoveMaps():
  """ This patches the rom before encoding, to clear the area where maps used to live. Moving maps is handled by the addr file. """
